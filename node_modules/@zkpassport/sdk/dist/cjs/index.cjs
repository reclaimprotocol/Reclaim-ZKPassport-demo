...
__export(index_exports, {
  ASEAN_COUNTRIES: () => import_utils5.ASEAN_COUNTRIES,
  EEA_COUNTRIES: () => import_utils5.EEA_COUNTRIES,
  EU_COUNTRIES: () => import_utils5.EU_COUNTRIES,
  MERCOSUR_COUNTRIES: () => import_utils5.MERCOSUR_COUNTRIES,
  SANCTIONED_COUNTRIES: () => import_utils5.SANCTIONED_COUNTRIES,
  SCHENGEN_COUNTRIES: () => import_utils5.SCHENGEN_COUNTRIES,
...
var import_registry2 = require("@zkpassport/registry");
...
var SolidityVerifier = class {
  ...
  static getParameters({
    ...
    const proofData = (0, import_utils2.getProofData)(proof.proof, (0, import_utils2.getNumberOfPublicInputs)(proof.name));
    let committedInputCounts = [];
    let committedInputs = [];
    for (const key in proof.committedInputs) {
      const committedInputCount = (0, import_utils2.getCommittedInputCount)(key);
      ...
      if (circuitName === "inclusion_check_issuing_country_evm" || circuitName === "inclusion_check_nationality_evm" || circuitName === "exclusion_check_issuing_country_evm" || circuitName === "exclusion_check_nationality_evm") {
        ...
        const proofType = (() => {
          switch (circuitName) {
            case "exclusion_check_issuing_country_evm":
              return import_utils2.ProofType.ISSUING_COUNTRY_EXCLUSION;
            case "exclusion_check_nationality_evm":
              return import_utils2.ProofType.NATIONALITY_EXCLUSION;
            case "inclusion_check_issuing_country_evm":
              return import_utils2.ProofType.ISSUING_COUNTRY_INCLUSION;
        ...
        const value = proof.committedInputs[circuitName];
        compressedCommittedInputs2 = import_utils2.ProofType.BIND.toString(16).padStart(2, "0") + (0, import_utils2.rightPadArrayWithZeros)((0, import_utils2.formatBoundData)(value.data), 500).map((x) => x.toString(16).padStart(2, "0")).join("");
    ...
    const parameterCommitments = (0, import_utils2.getParamCommitmentsFromOuterProof)(proofData).map(
    ...
    for (const commitment of parameterCommitments) {
      const committedInput = committedInputs.find((x) => {
        ...
        hashedInputs[0] = 0;
        return (0, import_utils3.bytesToHex)(hashedInputs) === commitment.replace("0x", "");
      });
      if (committedInput) {
...
// src/index.ts
var import_utils5 = require("@zkpassport/utils");
...
var ZKPassport = class {
  ...
  async handleEncryptedMessage(topic, request) {
    ...
    if (request.method === "accept") {
      ...
      this.topicToResults[topic] = (0, import_utils4.formatQueryResultDates)(request.params);
  ...
  async verify({
    ...
    if (uniqueIdentifier && uniqueIdentifierType && (uniqueIdentifierType === import_utils4.NullifierType.SALTED_MOCK || uniqueIdentifierType === import_utils4.NullifierType.NON_SALTED_MOCK) && !devMode) {
      verified = false;
      console.warn(
    ...
    if (verified) {
      const registryClient = new import_registry2.RegistryClient({ chainId: 11155111 });
