...
} from "./chunk-Q5BNOQ7G.js";
import {
  normaliseHash,
  strip0x
} from "./chunk-YNBT6XVD.js";
...
var log = debug("zkpassport:registry");
var CHAIN_CONFIG = {
  // Ethereum Mainnet
  1: {
...
var RegistryClient = class _RegistryClient {
  ...
  async isCertificateRootValid(root) {
    ...
    const response = await this.rpcRequest(
      this.rootRegistry,
      IS_ROOT_VALID_SIGNATURE + CERTIFICATE_REGISTRY_ID.toString(16).padStart(64, "0") + strip0x(root)
    );
    if (!response.ok) {
      throw new Error(
  ...
  async getHistoricalCertificateRoots(from, limit = DEFAULT_HISTORICAL_ROOTS_PAGE_SIZE) {
    ...
    CERTIFICATE_REGISTRY_ID.toString(16).padStart(64, "0")}${// startIndex parameter (uint256) padded to 32 bytes
    ...
    CERTIFICATE_REGISTRY_ID.toString(16).padStart(64, "0")}${// fromRoot parameter (bytes32) padded to 32 bytes
  ...
  async getCertificateRootDetails(root) {
    if (root) {
      ...
      const response2 = await this.rpcRequest(
        this.registryHelper,
        GET_ROOT_DETAILS_BY_ROOT_SIGNATURE + CERTIFICATE_REGISTRY_ID.toString(16).padStart(64, "0") + strip0x(root).padStart(64, "0")
      );
      if (!response2.ok) {
        throw new Error(
  ...
  async getHistoricalCircuitRoots(from, limit = DEFAULT_HISTORICAL_ROOTS_PAGE_SIZE) {
    ...
    const fromRoot = typeof from === "string" ? strip0x(from) : from ?? 1;
    const requestData = typeof fromRoot === "number" ? `${GET_HISTORICAL_ROOTS_SIGNATURE}${// registryId parameter (bytes32) padded to 32 bytes
    CIRCUIT_REGISTRY_ID.toString(16).padStart(64, "0")}${// startIndex parameter (uint256) padded to 32 bytes
    fromRoot.toString(16).padStart(64, "0")}${// limit parameter (uint256) padded to 32 bytes
    limit.toString(16).padStart(64, "0")}` : `${GET_HISTORICAL_ROOTS_BY_HASH_SIGNATURE}${// registryId parameter (bytes32) padded to 32 bytes
    CIRCUIT_REGISTRY_ID.toString(16).padStart(64, "0")}${// fromRoot parameter (bytes32) padded to 32 bytes
    fromRoot.padStart(64, "0")}${// limit parameter (uint256) padded to 32 bytes
    limit.toString(16).padStart(64, "0")}`;
    try {
      const response = await this.rpcRequest(this.registryHelper, requestData);
  ...
  async getCircuitRootDetails(root) {
    if (root) {
      ...
      const response2 = await this.rpcRequest(
        this.registryHelper,
        GET_ROOT_DETAILS_BY_ROOT_SIGNATURE + CIRCUIT_REGISTRY_ID.toString(16).padStart(64, "0") + strip0x(root).padStart(64, "0")
      );
      if (!response2.ok) {
        throw new Error(
  ...
  async isDocumentSupported(countryCode, issueDate, expirtyDate, type) {
    if (!DOCUMENT_SUPPORT_RULES_CACHE) {
      if (globalThis?.env?.USE_LOCAL_DOCUMENT_SUPPORT_RULES) {
        const response = await import("./document-support-rules-4SCW3X6S.js");
